...
{{-- <a href="#" class="btn btn-primary btn-sm"><i class="fas fa-file-excel "></i> Xuất file excel</a>
<a href="#" class="btn btn-info btn-sm"><i class="far fa-file-pdf"></i> Xuất file pdf</a> --}}
@endsection
@section('content')
<div ng-cloak>
                    ...
                    $html.= '<li class="dd-item dd3-item" data-id="'.$value['id'].'">
                      ...
                      <div class="dd3-content"> <span id="label_show'.$value['id'].'">'.$value['name'].'</span>
                        <span class="span-right">
                          ...
                          <a class="edit-button" href="'. route('Category.edit',$value['id']) .'" id="'.$value['id'].'" label="'.$value['name'].'"><i class="fas fa-pencil-alt"></i></a>
                          <a class="del-button confirm"  href="'. route('Category.delete',$value['id']) .'" id="'.$value['id'].'"><i class="fa fa-trash"></i></a></span>
                      </div>';
...
<script>
  $(document).ready(function(){
    ...
    $('.dd').on('change', function() {
      ...
      $.ajax({
        type: "POST",
        url: baseURL + '/admin/categories/nested-sort',
        data: {dataString},
        cache : false,
        headers: {
  ...
  app.controller('Category', function ($rootScope, $scope, $http) {
      $scope.loading = {};
      $scope.form = {}
      ...
      $('.link-button').on('click', function (e) {
          ...
          $.ajax({
              type: 'GET',
              url: "/admin/categories/" + cate_id + "/getDataForEdit",
              headers: {
                  'X-CSRF-TOKEN': CSRF_TOKEN
              },
              success: function (response) {
      ...
      $scope.submit = function () {
          ...
          $.ajax({
              type: 'POST',
              url: "{!! route('Category.add.homepage') !!}",
              headers: {
                  'X-CSRF-TOKEN': CSRF_TOKEN
              },
              data:{
